...
# Load the scaler
with open('models/rec_minmaxscaler.pkl', 'rb') as file:
    loaded_scaler = pickle.load(file)

# Loading price prediction model
# Load the trained model and encoders
with open('models/crop_price_predictor.pkl', 'rb') as model_file:
    price_model = pickle.load(model_file)

label_encoders = {}
for column in ['State', 'District', 'Market', 'Commodity']:
    with open(f'models/{column}_encoder.pkl', 'rb') as le_file:
...
app.config["SQLALCHEMY_DATABASE_URI"] = "mysql://root:@localhost/agritech"
...
def crop_prediction():
    ...
    if request.method == 'POST':
        ...
        if weather_fetch(city) != None:
            ...
            my_prediction = crop_recommendation_model.predict(scaled_features)
...
# Define the route for predicting crop prices
...
# Create a DataFrame for the input crop data
 ...
 # Transform the input data using the label encoders
    for feature in label_encoders:
        try:
            # Check if all values exist in the encoder's classes
            new_crop_data[feature] = new_crop_data[feature].apply(
                lambda x: label_encoders[feature].transform([x])[0] 
                if x in label_encoders[feature].classes_ else -1  # Set -1 for unseen values
            )
        except ValueError as e:
            # Handle unknown categories
            return "Error: One or more input values are not recognized in the training data."

    # Predict the price using the loaded model
    predicted_price = price_model.predict(new_crop_data[['State', 'District', 'Market', 'Commodity']])

    # Extract prices from the prediction
    modal_price = predicted_price[0][0]
    min_price = predicted_price[0][1]
    max_price = predicted_price[0][2]

    # Send the results as JSON response
    result = {
        'modal_price': f"{modal_price:.2f} Rs/quintal",
        'min_price': f"{min_price:.2f} Rs/quintal",
        'max_price': f"{max_price:.2f} Rs/quintal"
    }
    return render_template('crop-result.html', prediction=result, title=title)
